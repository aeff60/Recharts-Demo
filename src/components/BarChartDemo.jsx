// src/components/BarChartDemo.jsx
import {
    ...
    <div style={{
      ...
      {payload.map((p) => (
        <p key={p.dataKey} style={{ color: p.fill, margin: "2px 0" }}>
          {p.dataKey === "sales" ? "ยอดขาย" : "Revenue"}: <strong>{p.value.toLocaleString()}</strong>
        </p>
      ))}
    </div>
  );
    ...
    <>
      ...
      <div className="chart-body">
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={monthlyData} barGap={4} margin={{ top: 8, right: 16, left: -8, bottom: 0 }}>
